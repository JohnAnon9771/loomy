...
unless File.exist?('test/assets/base.png')
  Vips::Image.black(1024, 1024, bands: 3).linear([1],
                                                 [128, 128,
                                                  128]).bandjoin(255).cast(:uchar).write_to_file('test/assets/base.png')
end

unless File.exist?('test/assets/overlay.png')
  Vips::Image.black(500, 500, bands: 3).linear([1],
                                               [255, 0,
                                                0]).bandjoin(255).cast(:uchar).write_to_file('test/assets/overlay.png')
end

# Large assets (4200x4800)
unless File.exist?('test/assets/base_large.png')
  puts 'Generating large base asset (4200x4800)...'
  Vips::Image.black(4200, 4800, bands: 3).linear([1],
                                                 [50, 50,
                                                  50]).bandjoin(255).cast(:uchar).write_to_file('test/assets/base_large.png')
end

unless File.exist?('test/assets/overlay_large.png')
  puts 'Generating large overlay asset (2000x2000)...'
  Vips::Image.black(2000, 2000, bands: 3).linear([1],
                                                 [0, 255,
                                                  0]).bandjoin(255).cast(:uchar).write_to_file('test/assets/overlay_large.png')
end
